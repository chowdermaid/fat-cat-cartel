...
- The roomy hero introduction (information container at least 34rem wide) greets the current authenticated character, falling back to Discord display name for unlinked sessions or no name for visitors. Sydney time controls morning (5am-noon), afternoon (noon-6pm), evening, and the 1am-5am gremlin greeting. The gremlin message takes precedence over birthdays; saved profile birthdays take precedence over normal greetings, which add Saturday/Sunday wishes on weekends. The greeting reuses existing auth/profile data with no additional Firebase reads or calls, updates each minute and on visibility changes, and cleans up its timer/listener on unmount. Tests: `node --test tests/home-greeting.test.ts`.

- The Clubhouse replaces the placeholder in `src/features/home/components/hero/HomeHero.tsx`. The desktop hero uses a 35/65 introduction/Clubhouse split, excluding the gap, and stacks on mobile. Padding belongs to the introduction only; the Clubhouse fills the top, right, and bottom card edges, with the outer hero clipping its corners. On mobile the Clubhouse spans the card width below the introduction. The `/members` directory remains reachable through the existing hero link.
...
- Tune `CLUBHOUSE` in `src/features/home/constants.ts` for portrait sizes, tooltip delay, scene/artwork geometry, movement, and visible limits. Both layouts show 15 members (or the entire roster when smaller), including doorway transitions. Portrait diameter is 52px, reduced to 44px below 640px actual scene width. Hat IDs, labels, fixed rotations, dimensions, and placement live in `src/features/home/utils/clubhouseHats.ts`.
- Members and admins choose Cartel Fedora (default), Flat Cap, Witch Hat, Fat Cat Ears, or Tiny Crown using the Clubhouse hat dropdown and avatar preview in their existing profile editor. Save Profile commits the choice; Cancel discards it. Assets live in `src/assets/clubhouse/hats/`. Hats appear only in the Clubhouse and editor preview, with shared rendering and fitted placement per asset. Fedora adds 10 degrees clockwise to its embedded -15-degree tilt for a slight -5-degree final lean; other hats retain their native orientation. Cat ears sit lower and render behind the portrait inside its isolated stacking context; other hats stay in front. No random tilt. Movement bounds reserve the largest footprint across all hats, labels, and shadows; tooltip clearance uses the selected hat. Missing or unknown saved IDs render as fedora.
- The decorative `data-clubhouse-background` image in `src/features/home/components/clubhouse/CartelClubhouse.tsx` uses `src/assets/clubhouse/clubhouse-bg.png` by day and `clubhouse-bg-night.png` from 6pm to 6am Australia/Sydney time, including daylight saving. The mounted scene checks once per minute and on visibility changes. Scene height is at least 520px on desktop or 480px in narrow layouts and grows to the artwork's aspect ratio for wider scenes. Background cover uses left/bottom alignment to keep the doorway visible on narrow screens. The scene stretches with the hero and observes both width and height. Geometry uses the measured scene height with the same cover scaling and offset; carpet and doorway coordinates are normalized against the original 1983 by 793 artwork. The section retains an accessible name without a visible header.
...
- Stub mode seeds 20 members, including 12 clearly named development fixtures with empty and long biography cases. Its stable `membersLastUpdated` timestamp invalidates older eight-member stub caches using existing member-cache validation. Production data and cache keys are unchanged. Five fictional fixtures cover every hat; other profiles exercise the default.
