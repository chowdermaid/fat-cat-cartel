...
export function EncounterAveragesCard({
  members,
  encounters,
  contentType,
}: Props) {
  ...
  const rows = encounters.map((enc) => {
    ...
    const avg =
      parses.length > 0
        ? parses.reduce((a, b) => a + b, 0) / parses.length
        : null;
  ...
  useEffect(() => {
    ...
    const bars = Array.from(
      listRef.current.querySelectorAll<HTMLElement>(".ea-bar"),
    );
    bars.forEach((el, i) => {
      animate(el, {
